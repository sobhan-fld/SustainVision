...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Faster R-CNN ResNet-18 (ImageNet) + FPN VOC baseline (finetune backbone + heads)"
    )
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="original_fasterrcnn_resnet18_imagenet/outputs",
    )
...
def build_model(num_classes: int, min_size: int, max_size: int) -> FasterRCNN:
    ...
    try:
        ...
        try:
            backbone = resnet_fpn_backbone(
                "resnet18",
                weights=weights,
                trainable_layers=5,  # finetune entire backbone
            )
        except TypeError:
            # Older torchvision
            backbone = resnet_fpn_backbone("resnet18", pretrained=True, trainable_layers=5)
    except Exception:
        # Fallback for older torchvision where weights enums may not exist.
        backbone = resnet_fpn_backbone("resnet18", pretrained=True, trainable_layers=5)
...
def _find_last_epoch_checkpoint(output_dir: Path) -> Optional[Path]:
    candidates = list(output_dir.glob("fasterrcnn_resnet18fpn_imagenet_voc10_epoch*.pt"))
    if not candidates:
        return None
...
def main() -> None:
    ...
    if args.eval_only or args.eval_all:
        ...
        if args.eval_all:
            ckpts = sorted(output_dir.glob("fasterrcnn_resnet18fpn_imagenet_voc10_epoch*.pt"))
            if not ckpts:
    ...
    if args.track_emissions and EmissionsTracker is not None:
        tracker = EmissionsTracker(
            project_name="original_fasterrcnn_resnet18_imagenet_voc12",
            log_level="warning",
            save_to_file=False,
        )
        tracker.start()
    ...
    for epoch in range(args.epochs):
        ...
        ckpt_path = output_dir / f"fasterrcnn_resnet18fpn_imagenet_voc10_epoch{epoch + 1}.pt"
