...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Faster R-CNN ResNet-34 (SupCon CIFAR100 via timm) + FPN VOC10 baseline "
            "(backbone frozen; train detection heads only)."
        )
    )
    parser.add_argument(
        "--data-root",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="SGD learning rate for detection heads (backbone is frozen).",
    )
    parser.add_argument(
        "--min-size",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="original_fasterrcnn_resnet34_supcon_cifar100_frozen_backbone/outputs",
    )
    ...
    parser.add_argument(
        "--hf-backbone",
        type=str,
        default="resnet34_supcon_cifar100",
        help="timm model name to load (pretrained=True).",
    )
...
def build_model(
    *,
    ...
    # Freeze backbone completely (and keep it in eval mode to avoid BN drift).
...
def _find_last_epoch_checkpoint(output_dir: Path) -> Optional[Path]:
    candidates = list(output_dir.glob("fasterrcnn_resnet34fpn_supcon_cifar100_voc10_frozen_epoch*.pt"))
    if not candidates:
        return None
...
def compute_val_loss(model: FasterRCNN, loader: DataLoader, device: torch.device) -> float:
    # For consistent loss computation, keep detector in train mode but freeze BN in backbone.
    model.train()
    model.backbone.eval()
    val_loss = 0.0
    samples = 0
...
def main() -> None:
    ...
    model = build_model(
        ...
        hf_backbone=str(args.hf_backbone),
    ...
    if args.eval_only or args.eval_all:
        ...
        if args.eval_all:
            ckpts = sorted(output_dir.glob("fasterrcnn_resnet34fpn_supcon_cifar100_voc10_frozen_epoch*.pt"))
            if not ckpts:
    ...
    # Train only parameters that require grad (detector heads).
    ...
    if args.track_emissions and EmissionsTracker is not None:
        tracker = EmissionsTracker(
            project_name="original_fasterrcnn_resnet34_supcon_cifar100_voc10_frozen",
            log_level="warning",
            save_to_file=False,
        )
        tracker.start()
    ...
    for epoch in range(args.epochs):
        model.train()
        # Keep frozen backbone in eval to avoid BatchNorm drift.
        model.backbone.eval()
        ...
        ckpt_path = output_dir / f"fasterrcnn_resnet34fpn_supcon_cifar100_voc10_frozen_epoch{epoch + 1}.pt"
