...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Faster R-CNN ResNet-18 (ImageNet) + FPN VOC baseline (frozen backbone by default)"
    )
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="original_fasterrcnn_resnet18_imagenet_frozen_backbone/outputs",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If set, finetune the ResNet-18+FPN backbone. By default the backbone is frozen and only detection heads are trained.",
    )
    parser.add_argument(
        "--eval-only",
        action="store_true",
...
def build_model(num_classes: int, train_backbone: bool, min_size: int, max_size: int) -> FasterRCNN:
    # ImageNet-pretrained ResNet-18 backbone + FPN.
    trainable_layers = 5 if train_backbone else 0
    try:
        ...
        try:
            ...
            # Older torchvision
            backbone = resnet_fpn_backbone("resnet18", pretrained=True, trainable_layers=trainable_layers)
    except Exception:
        backbone = resnet_fpn_backbone("resnet18", pretrained=True, trainable_layers=trainable_layers)
...
def _find_last_epoch_checkpoint(output_dir: Path) -> Optional[Path]:
    candidates = list(output_dir.glob("fasterrcnn_resnet18fpn_imagenet_voc10_frozen_epoch*.pt"))
    if not candidates:
        return None
...
def main() -> None:
    ...
    if args.eval_only or args.eval_all:
        ...
        if args.eval_all:
            ckpts = sorted(output_dir.glob("fasterrcnn_resnet18fpn_imagenet_voc10_frozen_epoch*.pt"))
            if not ckpts:
    ...
    if args.track_emissions and EmissionsTracker is not None:
        tracker = EmissionsTracker(
            project_name="original_fasterrcnn_resnet18_imagenet_voc12_frozen",
            log_level="warning",
            save_to_file=False,
        )
        tracker.start()
    ...
    for epoch in range(args.epochs):
        ...
        ckpt_path = output_dir / f"fasterrcnn_resnet18fpn_imagenet_voc10_frozen_epoch{epoch + 1}.pt"
