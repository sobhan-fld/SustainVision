...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Faster R-CNN VGG16 VOC-10 baseline")
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="original_fasterrcnn_frozen_backbone/outputs",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If set, finetune the VGG16 backbone. By default the backbone is frozen and only detection heads are trained.",
    )
    parser.add_argument(
        "--eval-only",
        action="store_true",
...
def _parse_voc_target(target: Dict, orig_w: int, orig_h: int) -> Tuple[torch.Tensor, torch.Tensor]:
...
class VOC10Detection(tv_datasets.VOCDetection):
    ...
    def __getitem__(self, idx: int):
        ...
        boxes_t, labels_t = _parse_voc_target(target, orig_w, orig_h)
        return img_t, {
            "boxes": boxes_t,
            "labels": labels_t,
...
def build_model(num_classes: int, train_backbone: bool) -> FasterRCNN:
...
def _voc_ap_continuous(rec: torch.Tensor, prec: torch.Tensor) -> float:
    """
    VOC-style continuous AP (precision envelope + area under PR curve).
    """
...
def evaluate_voc_map50(
    ...
    """
    Compute VOC mAP@0.5 (continuous AP).
    Assumes labels are in 1..K (0 reserved for background).
    """
    model.eval()
...
def _find_last_epoch_checkpoint(output_dir: Path) -> Optional[Path]:
    candidates = list(output_dir.glob("fasterrcnn_vgg16_voc10_epoch*.pt"))
    if not candidates:
        return None
...
def main() -> None:
    ...
    model = build_model(num_classes=len(VOC10_CLASSES) + 1, train_backbone=bool(args.train_backbone))
    model.to(device)
    ...
    if args.eval_only or args.eval_all:
        ...
        if args.eval_all:
            ckpts = sorted(output_dir.glob("fasterrcnn_vgg16_voc10_epoch*.pt"))
            if not ckpts:
    ...
    if args.track_emissions and EmissionsTracker is not None:
        tracker = EmissionsTracker(
            project_name="original_fasterrcnn_vgg16_voc12",
            log_level="warning",
            save_to_file=False,
        )
        tracker.start()
    ...
    for epoch in range(args.epochs):
        ...
        ckpt_path = output_dir / f"fasterrcnn_vgg16_voc10_epoch{epoch + 1}.pt"
